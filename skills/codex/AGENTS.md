...
Registration inside Codex binds the current session locally by default. The
single bridge owned by `scope serve` resumes the addressed Codex session,
retries transient failures, and acknowledges after provider acceptance. Use
`scope --json bridge bind AGENT_ID --provider codex --session UUID` when the
session id must be supplied explicitly, and `bridge status` to inspect safe
connection state. Use native Codex messaging for siblings already running in
one harness.
